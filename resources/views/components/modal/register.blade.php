        ...
        <div class="border-0 rounded-xl shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="relative p-6 flex-auto mx-10">
                    ...
                    @if ($errors->has('name'))
                        <p class="text-red-500 mb-5 text-sm">{{ $errors->first('name') }}</p>
                    @endif
                    </div>
                    <div class="mb-4">
                    ...
                    @if ($errors->has('email'))
                        <p class="text-red-500 mb-5 text-sm">{{ $errors->first('email') }}</p>
                    @endif
                    </div>
                    <div>
                        <label class="block text-grey-darker text-sm mb-2" for="password_confirmation">Password</label>
                        <input name="password" class="appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 placeholder-serv-text text-xs mb-3" id="password_confirmation" type="password" placeholder="At least 8 characters" required>

                    @if ($errors->has('password'))
                        <p class="text-red-500 mb-5 text-sm">{{ $errors->first('password') }}</p>
                    @endif
                    </div>
                    <div>
                        <label class="block text-grey-darker text-sm mb-2" for="password">Confirm Password</label>
                        <input name="password_confirmation" class="appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 placeholder-serv-text text-xs mb-3" id="password" type="password" placeholder="At least 8 characters" required>

                    @if ($errors->has('password_confirmation'))
                        <p class="text-red-500 mb-5 text-sm">{{ $errors->first('password_confirmation') }}</p>
                    @endif
                    </div>
